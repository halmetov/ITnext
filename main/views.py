...
def salomber(request):
    limit = int(request.GET.get('limit', 2))
    hazrgi  = int(request.GET.get('page', 1))
    stop = hazrgi * limit
    start = stop - limit



    blog = Blog.objects.filter()[start:stop]

    son = Blog.objects.count()
    ...
    return render(request, 'pagin.html', {'ddd': blog,'hazrgi':hazrgi, 'pred': pred, 'sled': sled})
...
def blog_detailHandler(request, blog_detail_id):
    if request.POST:
        ...
        return redirect(f'/blog/{int(blog_detail_id)}')
